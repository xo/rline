#!/bin/sh
# Build the probe that prints what isocline's tty.c and tty_esc.c return.
#
...
# whole library, so it reaches the static functions in tty.c and tty_esc.c.
set -eu
...
${CC:-gcc} -std=c99 -O2 \
  ...
  -o "$out/probe-tty" \
  "$root/tools/probe-tty.c"

echo "built $out/probe-tty"
