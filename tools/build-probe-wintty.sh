#!/bin/sh
# Build the probe that prints what isocline's the Windows key encoding in tty.c return.
#
...
# whole library, so it reaches the static functions in the Windows key encoding in tty.c.
set -eu
...
${CC:-gcc} -std=c99 -O2 \
  ...
  -o "$out/probe-wintty" \
  "$root/tools/probe-wintty.c"

echo "built $out/probe-wintty"
