#!/bin/sh
# Build the probe that prints what isocline's the file name completion in completers.c return.
#
...
# whole library, so it reaches the static functions in the file name completion in completers.c.
set -eu
...
${CC:-gcc} -std=c99 -O2 \
  ...
  -o "$out/probe-filenames" \
  "$root/tools/probe-filenames.c"

echo "built $out/probe-filenames"
