#!/bin/sh
# Build the probe that prints what isocline's the escape decoder, for differential fuzzing return.
#
...
# whole library, so it reaches the static functions in the escape decoder, for differential fuzzing.
set -eu
...
${CC:-gcc} -std=c99 -O2 \
  ...
  -o "$out/probe-ttyfuzz" \
  "$root/tools/probe-ttyfuzz.c"

echo "built $out/probe-ttyfuzz"
