#!/bin/sh
# Build the probe that prints what isocline's wcwidth.c and stringbuf.c return.
#
...
# whole library, so it reaches the static functions in stringbuf.c.
set -eu
...
${CC:-gcc} -std=c99 -O2 \
  ...
  -o "$out/probe-stringbuf" \
  "$root/tools/probe-stringbuf.c"

echo "built $out/probe-stringbuf"
