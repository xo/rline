// The linter lives in a module of its own so that it does not raise the Go
// version of the library.
//
// golangci-lint needs Go 1.26. The library needs 1.25.0, which is what
// golang.org/x/sys asks for and what FreeBSD 15.1 ships. A tool directive in
// the main module would have made the library need 1.26 as well, for a
// program no caller of the library ever builds.
//
// Run it from the top of the repository:
//
//	go -C tools/lint tool golangci-lint run ../../...
module github.com/xo/rline/tools/lint
