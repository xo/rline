#!/bin/sh
# Build the probe that prints what isocline's history.c and undo.c return.
#
...
# whole library, so it reaches the static functions in history.c and undo.c.
set -eu
...
${CC:-gcc} -std=c99 -O2 \
  ...
  -o "$out/probe-history" \
  "$root/tools/probe-history.c"

echo "built $out/probe-history"
