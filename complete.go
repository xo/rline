...
// Offering completions from inside the edit loop: asking the completer,
// applying the only answer when there is only one, and drawing the menu when
// there are several.
//
// The menu is not a separate drawing surface. It writes into the editor's
// extra buffer and calls the same refresh the rest of the editor uses, so
// what appears below the line is the same mechanism as the hint help. What it
// does have of its own is a key loop: it reads the tty directly rather than
// going through readKey, which is what the C does.
//
...
// leftover help would eat the start of the menu. It cannot happen, because
// the edit loop clears the hint and its help before it dispatches the key
// that opens the menu, and the C clears it in the same place. The menu
// therefore says nothing about the hint, and that is deliberate rather than
// an oversight.
//
// The list itself, and the completers that fill it, are ported in
