...
// TestDecoderPort replays every recorded decode and checks that the Go port
// answers the same.
func TestDecoderPort(t *testing.T) {
	if *update {
		regenerateTTY(t)
	}
...
func checkTTYLine(t *testing.T, line string) (string, string) {
	...
	case "escresp":
		...
		term := newDecoder(&idleReader{bytes: in})
...
// TestDecoderPushedCodeComesBackUnchanged checks that a key pushed back is
...
func TestDecoderPushedCodeComesBackUnchanged(t *testing.T) {
	t.Parallel()
...
// TestDecoderPushedCodesAreReadInReverse records that the code buffer is a stack,
// so the key pushed last is read first.
func TestDecoderPushedCodesAreReadInReverse(t *testing.T) {
	t.Parallel()
	term := newDecoder(&idleReader{})
...
// TestDecoderPushbackStopsAtTheLimit checks that neither buffer grows without
// bound. Anything past the limit is dropped, as in the C code.
func TestDecoderPushbackStopsAtTheLimit(t *testing.T) {
	t.Parallel()
	term := newDecoder(&idleReader{})
...
// TestDecoderReadEndsAtKeyNone checks the blocking read, which returns key.None
// once the input is finished.
func TestDecoderReadEndsAtKeyNone(t *testing.T) {
	t.Parallel()
	term := newDecoder(&idleReader{bytes: []byte("hi")})
...
// TestDecoderSetEscDelay checks that the waits can be changed, which the timing
// tests will need once a clock can be injected.
func TestDecoderSetEscDelay(t *testing.T) {
	t.Parallel()
	term := newDecoder(&idleReader{})
...
// TestDecoderDecodesAcrossReads checks that a sequence split over several reads
...
func TestDecoderDecodesAcrossReads(t *testing.T) {
	t.Parallel()
	term := newDecoder(&idleReader{bytes: []byte("\x1b[1;5A")})
	term.setEscDelay(0, 0)
	got, ok := term.readTimeout(0)
	if !ok {
